import { component$, useVisibleTask$ } from "@builder.io/qwik";
import { useLocation } from "@builder.io/qwik-city";

const SECRET_CODE = "thailand2024";

export default component$(() => {
  const loc = useLocation();
  const code = loc.url.searchParams.get("code");

  useVisibleTask$(() => {
    if (code === SECRET_CODE) {
      document.body.classList.add("authenticated");
    } else {
      document.body.classList.remove("authenticated");
    }
  });

  if (code !== SECRET_CODE) {
    return (
      <div class="mt-12 text-center">
        <h2>404 page not found</h2>
      </div>
    );
  }

  return (
    <div class="relative mx-auto my-12 max-w-[620px] overflow-hidden px-8">
      <h2 class="relative z-[1] pb-5 text-left text-5xl text-black">
        Kochana Martyno
      </h2>
      <h3 class="relative z-[1] mb-6 text-2xl text-black">
        Z okazji urodzin dostajesz od swojego kochającego chłopaka Macieja kartę
        małego podróżnika.
      </h3>
      <p class="relative z-[1] mb-6 text-2xl text-black">
        Kwota do wykorzystania wynosi 832,85 zł. <br /> Środki można przeznaczyć
        na dowolny cel podróżniczy (SUGESTIA W TLE) po daniu buziaczka.
      </p>
      <div class="relative z-[1] my-8 mb-2 font-[sans-serif] text-xl font-bold">
        <div class="relative m-auto h-48 w-80 rounded-xl bg-gradient-to-r from-gray-500 to-gray-400 text-white shadow-2xl transition-transform sm:h-56 sm:w-96 sm:hover:scale-105">
          <div class="absolute top-4 w-full px-6 text-left sm:top-8">
            <div class="flex justify-end">
              <img
                width="56"
                height="56"
                class="object-contain"
                src="data:image/png;base64,"
              />
            </div>
            <div class="pt-8">
              <p class="text-[10px] font-light">Card Number</p>
              <p class="tracking-more-wider text-lg font-medium">
                4312 567 7890 7864
              </p>
            </div>
            <div class="pt-2">
              <p class="text-[10px] font-light">Name</p>
              <p class="text-xs font-medium tracking-widest">
                Martyna Kapuścińska
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
});
